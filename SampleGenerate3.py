...
def main(argv):
    ...
    output_R1 = open(r1_final, 'wb')
    output_R2 = open(r2_final, 'wb')

    #loop strains
    for strainId, ssCov in strainCov[n].items():
        print("\tRequesting " +str(ssCov) + "coverage for " +  strainId)
        totalLen = 0    
        ...
        print('\tGenerated ' + str(r1_n) + ' pairs for ' + strainId + ', ' + str(effective_cov) +  ' coverage')

        if r1_n != r2_n:
            print('Error: paired-end counts do not match ' + str(r1_n) + ' vs ' + str(r2_n))
            sys.exit(1)

        with open(r1_tmp,'rb') as f:
            output_R1.write(f.read())
                
        with open(r2_tmp,'rb') as f:
